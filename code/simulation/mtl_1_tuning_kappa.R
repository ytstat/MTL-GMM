...
filename = paste(relative_path, "/output/simulation/mtl_1_tuning_kappa/", seed, ".RData", sep = "")
...
# ---------------------------------------------------------
# Simulation 1 with different kappa parameters used in MTL-GMM
# ---------------------------------------------------------
# See Section S.5.1.8 in the paper
...
error_mtlgmm <- rep(list(matrix(nrow = nrow(C_matrix), ncol = 7, dimnames = list(NULL, c("w", "mu", "beta", "delta", "Sigma", "misclustering_max", "misclustering_avg")))), length(kappa_list))
...
for (i in 1:nrow(C_matrix)) {
  ...
  # MTL-GMM
  fit <- list()
  for (j in 1:length(kappa_list)) {
    fit[[j]] <- mtlgmm(x = x, kappa = kappa_list[j], initial_method = "EM", ncores = ncores,
                       lambda_choice = "cv", step_size = "lipschitz", tol = 1e-3, trim = 0.1,
                       cv_nfolds = 10, cv_upper = "auto", cv_lower = 0.01, cv_length = 10)
  }

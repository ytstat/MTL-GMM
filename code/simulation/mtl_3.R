...
filename = paste(relative_path, "/output/simulation/mtl_3/", seed, ".RData", sep = "")
...
# ---------------------------------------------------------
# Simulation 3 of MTL in the paper
# ---------------------------------------------------------
# See Section S.5.1.3 in the paper
...
for (i in 1:nrow(C_matrix)) {
  data_bundle <- data_generation(K = 10, outlier_K = C_matrix[i, 2], h = C_matrix[i, 1], n = 600, p = 5, simulation_no = "MTL-3")
  ...
  # MTL-GMM
  fit <- mtlgmm(x = x, kappa = 1/3, initial_method = "EM", ncores = ncores, cv_nfolds = 10,
                cv_upper = "auto", cv_lower = 0.01, cv_length = 10,
                lambda_choice = "cv", step_size = "lipschitz", tol = 1e-3)

...
filename = paste(relative_path, "/output/simulation/mtl_epsilon/", seed, ".RData", sep = "")
...
# ---------------------------------------------------------
# Simulation 5 of MTL in the paper, a variant of Simulation 1 with varying epsilon values
# ---------------------------------------------------------
...
for (i in 1:nrow(C_matrix)) {
  data_bundle <- data_generation(K = K, outlier_K = floor(C_matrix[i, 2]*K), h = C_matrix[i, 1], n = 600, p = 15, simulation_no = "MTL-epsilon")
